...
int main (int argc, char *argv[])
{
	...
	while(!stop)
	{
		...
		while(run)
		{
			...
			DeleteTextBoxLines (panelId, PANEL_STATUSBOX, 0, -1);
			char msg[64];
			sprintf(msg,"%i seconds until next acquisition",timestep - (tnow%(timestep*1000))/1000);	
			if( loop > 1 ) SetCtrlVal(panelId,PANEL_STATUSBOX,msg);
			for( int jjj=0; ( jjj < (int) (timestep - 1.0) ) && ( loop < 2 ); jjj++ )
			{
				
				if(!jjj) DAQmxWriteDigitalLines(taskHandle,1,1,10.0,DAQmx_Val_GroupByChannel,sh_open,NULL,NULL);

				DeleteTextBoxLines (panelId, PANEL_STATUSBOX, 0, -1);
				sprintf(msg,"--INITIALIZING-- data cycle 0 will start in %i seconds.", (int) ( timestep - (double) jjj ) );
				SetCtrlVal(panelId,PANEL_STATUSBOX,msg);
				tLast=Timer()*1000.0;
				wait( 1.0 );
			}
			if(isRunning && ((tnow%(timestep*1000))<=1000))
			{
				...
				tLast=Timer()*1000.0;
				wait(32); //KDW 2021-2-1 moved this here from line 824  TODO***
				strcpy(command,"/?COMMAND=data:source+");
				strcat(command,ch[0]);
				...
				//plot the background-subtracted waveform
				DeleteGraphPlot (panelId,PANEL_GRAPH, -1, VAL_IMMEDIATE_DRAW);	
					...
					if(wferr>=0 && srcerr>=0)
					{
						...
						for(i=0; (i<500)&&(run); i++)
						{
							...
							if((waveform!=NULL)&&(len<=strlen(dat)))
							{
								...
								laser2[i]=1.0e1*wavinfo[12]*(laser1[i]-wavinfo[14])+wavinfo[13];
							}
						}
						len=len+strlen(waveform)+1;
					}
				...
				time_now=time(NULL);
				if(!eventnum) tzero=time_now;
				if(run) scopetracehandle=PlotXY(panelId,PANEL_GRAPH,t,voltfit,500,VAL_DOUBLE,VAL_DOUBLE,VAL_THIN_LINE,VAL_NO_POINT,0,0,VAL_RED);
				...
				*(timeaxis+eventcount)=(lifeparams[7]-tzero)/3600.0;
				...
				if(run ) PlotXY(panelId,PANEL_TIMEHIST,timeaxis,lifetime,eventcount+1, VAL_DOUBLE,VAL_DOUBLE,VAL_SCATTER,VAL_DOTTED_EMPTY_CIRCLE,0,1,VAL_RED);
				if(checkbox_2 && run)
				{
				...
				if(run) xyattr=SetCtrlAttribute(panelId,PANEL_TIMEHIST,ATTR_XAXIS_GAIN,10.0);
				eventcount=eventnum;
				...
				for(j=0; j<3 && 0 ; j++)
				{
					...
					if(checkbox_2)
					{
						if(j==1)
						{
							...
							talkToScope("/?COMMAND=horizontal:main:scale+40e-9",src,gHandle,5000,0);
							//DeleteTextBoxLines (panelId, PANEL_STATUSBOX, 0, -1);
							//SetCtrlVal(panelId,PANEL_STATUSBOX,"Waiting");
							tLast=Timer()*1000.0;
							wait(65); //TODO****
						}
					}
					...
					if(j==0)
					{
						...
						if(j==0)
						{
							...
							//printf("waiting");
							//wait(32);	   KDW 2021-2-1 uncommented and moved this to line 836
							status=0;
							while(status==0)
							{
				...
				if( weekend && eventnum%10 == 0 )
				{
					...
					for( int iii=0; iii< (int) long_wait; iii++ )
					{
						DeleteTextBoxLines (panelId, PANEL_STATUSBOX, 0, -1);
						sprintf(msg,"Fiber-saving. --SHUTTER CLOSED-- will reopen in %i seconds.", (int) ( long_wait - (double)iii ) );
						SetCtrlVal(panelId,PANEL_STATUSBOX,msg);
						tLast=Timer()*1000.0;
						wait( 1.0 );
					}
					...
					talkToScope(command,dat,gHandle,3848,2);
					for( int jjj=0; jjj< (int) timestep; jjj++ )
					{
						DeleteTextBoxLines (panelId, PANEL_STATUSBOX, 0, -1);
						sprintf(msg,"Fiber-saving. --SHUTTER OPEN-- Cycle paused, will resume in %i seconds.", (int) ( timestep - (double) jjj ) );
						SetCtrlVal(panelId,PANEL_STATUSBOX,msg);
						tLast=Timer()*1000.0;
						wait( 1.0 );
					}
